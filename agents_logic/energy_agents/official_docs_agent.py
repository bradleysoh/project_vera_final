...
from agents_logic.energy_agents.domain_config import DOMAIN_CONFIG

_METADATA_SCHEMA = DOMAIN_CONFIG.get("metadata_schema", {})


@vera_agent("Energy Official Docs Agent")
def run(state: GraphState) -> dict:
    ...
    if intent not in ("spec_retrieval", "cross_reference", ""):
        print(f"[Energy Official Docs Agent] ⏭️ Fast-fail: intent='{intent}' is not spec-related")
        return {}
    ...
    print(f"[Energy Official Docs Agent] {len(result.documents)} docs → {len(facts)} structured facts")
