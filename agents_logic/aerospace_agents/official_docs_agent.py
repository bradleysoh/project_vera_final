...
@vera_agent("Aerospace Official Docs Agent")
def run(state: GraphState) -> dict:
    ...
    if intent not in ("spec_retrieval", "cross_reference", ""):
        print(f"[Aerospace Official Docs Agent] ⏭️ Fast-fail: intent='{intent}' is not spec-related")
        return {}
    ...
    print(f"[Aerospace Official Docs Agent] {len(result.documents)} docs → {len(facts)} structured facts")
