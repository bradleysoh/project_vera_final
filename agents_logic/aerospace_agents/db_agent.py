"""
================================================================================
Aerospace DB Agent — Domain-Generic NL-to-SQL Retrieval (Precision Mode)
================================================================================
DOMAIN: aerospace (also reusable for any domain with .db files)
    ...
    for attempt in range(_MAX_SQL_RETRIES):
        try:
            ...
            if any(kw in last_error.lower() for kw in [
                ...
                print(
                    f"[Aerospace DB Agent] ⚠️ SQL error (attempt {attempt + 1}/{_MAX_SQL_RETRIES}): "
                    f"{last_error[:100]}"
                )
                ...
                try:
                    ...
                    print(f"[Aerospace DB Agent] 🔧 Corrected SQL: {current_sql[:100]}")
...
def run(state: GraphState) -> dict:
    ...
    if intent not in ("db_query", "cross_reference", "spec_retrieval", ""):
        print(f"[Aerospace DB Agent] ⏭️ Fast-fail: intent='{intent}' is not DB-related")
        return {}
    ...
    if not db_paths:
        ...
        print(f"[Aerospace DB Agent] ⚠️ {msg}")
        return {
    ...
    print(f"[Aerospace DB Agent] Found {len(db_paths)} database(s) for '{user_domain}': {db_names}")
    ...
    print(f"[Aerospace DB Agent] 🎯 Target Entity: {entity} (type: {entity_type})")
    ...
    if RETRIEVAL_MODE == "fast" and not is_general:
        ...
        for db_path in db_paths:
            ...
            try:
                ...
                print(f"[Aerospace DB Agent] Schema scan error for {db_name}: {e}")
        ...
        for db_path, db_name, sql in sql_queries:
            try:
                ...
                if rows:
                    ...
                    print(f"[Aerospace DB Agent] ✅ {db_name}: {len(rows)} rows (deterministic)")
            except Exception as e:
                print(f"[Aerospace DB Agent] ❌ {db_name}: {e}")
    ...
    print(f"[Aerospace DB Agent] Generated SQL: {sql}")
    ...
    for db_path in db_paths:
        ...
        if last_err and "no such table" in last_err.lower():
            print(f"[Aerospace DB Agent] ⏭️ {db_name}: table not found, trying next DB")
            continue
        ...
        if rows:
            ...
            print(f"[Aerospace DB Agent] ✅ {db_name}: {len(rows)} rows")
