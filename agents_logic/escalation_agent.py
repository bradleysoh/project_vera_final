...
def run(state: GraphState) -> dict:
    ...
    is_domain_scope = bool(state.get("required_domain"))

    if is_domain_scope:
        required = state["required_domain"]
        message = (
            f"⚠️ This request requires the '{required}' domain.\n\n"
            f"Please switch your **User Domain** to `{required}` and run the query again."
        )
        return {
            "generation": message,
            "documents": [],
            "_thinking": f"Scope guard: request requires '{required}' domain but user is in '{user_domain}'.",
        }
