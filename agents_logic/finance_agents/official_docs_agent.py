...
from agents_logic.finance_agents.domain_config import DOMAIN_CONFIG

_METADATA_SCHEMA = DOMAIN_CONFIG.get("metadata_schema", {})


@vera_agent("Finance Official Docs Agent")
def run(state: GraphState) -> dict:
    ...
    if intent not in ("spec_retrieval", "cross_reference", ""):
        print(f"[Finance Official Docs Agent] ⏭️ Fast-fail: intent='{intent}' is not spec-related")
        return {}
    ...
    print(f"[Finance Official Docs Agent] {len(result.documents)} docs → {len(facts)} structured facts")
